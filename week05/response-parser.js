...
class ChunkedBodyParser {
  constructor() {
    ...
    this.contentLine = '';
    this.chunked = [];
    this.current = this.WAITING_CHUNK_SIZE;
      ...
      case this.WAITING_CHUNK_SIZE:
        if (char !== '\r') {
          this.currentChunkLength =
            this.currentChunkLength * 10 + char.charCodeAt(0) - '0'.charCodeAt(0);
        } else {
          if (this.currentChunkLength === 0) {
            this.current = this.FINISHED_BODY_PARSE;
          } else {
            this.current = this.WAITING_CHUNK_SIZE_END;
          }
        }
        break;
      case this.WAITING_CHUNK_SIZE_END:
        if (char === '\n') this.current = this.WAITING_CONTENT;
        break;
      case this.WAITING_CONTENT:
        if (this.currentChunkLength === 0) {
          if (char === '\r') {
            ...
            this.contentLine = '';
            this.current = this.WAITING_CONTENT_END;
          }
        } else {
          this.currentChunkLength--;
